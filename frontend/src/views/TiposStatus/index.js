...
import tiposStatusApi from "../../services/tiposStatusApi";
...
function TiposStatus () {
    ...
    const onLoad = async () => {
        let response = await tiposStatusApi._all();
    ...
    const onDelete = async (id) => {
        await tiposStatusApi._delete(id)
        onLoad();
    }
    ...
    const onSave = async (item) =>{
        if(item.id === 0) {
            // create
            return await tiposStatusApi._create(item)
        } else {
            // update
            return await tiposStatusApi._update(item.id, item)
        }
    }
...
function DataTable( info ){
    ...
    return (
        ...
        <div className="box">
            <div className="box-header">
                <h3 className="box-title">Tipos de Status</h3>
            </div>
